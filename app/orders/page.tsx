...
> = {
  PENDING_ADMIN: {
    ...
    description:
      "Your design has been submitted and is waiting for admin review.",
    step: 1,
  },
  ASSIGNED_TO_SUPPLIER: {
    ...
    description:
      "Your order has been approved and assigned to a supplier for production.",
    step: 2,
  },
  COMPLETED_BY_SUPPLIER: {
    ...
    description:
      "Your order has been completed by the supplier and is on its way!",
    step: 3,
  },
  REJECTED: {
    label: "Rejected",
    ...
    description:
      "Unfortunately, this design could not be approved. Please redesign and resubmit.",
    step: 0,
  },
};
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] font-sans">
      ...
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-xl border-b border-gray-100 shadow-sm">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              href="/editor"
              className="bg-[#A1FF4D] text-[#1B2412] px-4 py-2 rounded-xl font-black text-xs hover:bg-[#8ee53f] transition-all flex items-center gap-1.5"
            >
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        ...
        {showSuccess && (
          <div className="mb-8 bg-[#A1FF4D] rounded-3xl p-6 flex items-center gap-5 shadow-lg shadow-[#A1FF4D]/20 animate-in slide-in-from-top-4 fade-in duration-500">
            ...
            <div className="flex-1">
              <h2 className="text-[#1B2412] font-black text-xl">
                Design Submitted Successfully! 🎉
              </h2>
              <p className="text-[#2B3220]/80 font-medium text-sm mt-0.5">
                Your order is now in the review queue. We'll notify you once
                it's approved and sent to production.
              </p>
            </div>
            <button
              onClick={() => setShowSuccess(false)}
              className="text-[#1B2412]/50 hover:text-[#1B2412] transition-colors flex-shrink-0"
            >
              <XCircle size={20} />
            </button>
          </div>
        ...
        <div className="mb-8">
          <h1
            className="text-4xl font-black text-[#111] uppercase tracking-tighter"
        ...
        {orders.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-gray-500 font-medium max-w-sm mb-8">
              Design your first custom product and it will appear here for you
              to track.
            </p>
            <Link
              href="/editor"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
            ...
            <div className="lg:col-span-2 space-y-3">
              ...
              {orders.map((order) => {
                const cfg =
                  STATUS_CONFIG[order.status] || STATUS_CONFIG.PENDING_ADMIN;
                const Icon = cfg.icon;
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-gray-400 font-bold">
                          {order.variants?.color} • {order.variants?.view}
                        </p>
                        <div
                          className={`inline-flex items-center gap-1 mt-1.5 px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider ${cfg.bg} ${cfg.color}`}
                        >
                          <Icon size={9} />
                          {cfg.label}
                        </div>
                      ...
                      <ArrowRight
                        size={14}
                        className={`flex-shrink-0 transition-colors ${isSelected ? "text-[#A1FF4D]" : "text-gray-200"}`}
                      />
                    </div>
                  </button>
                );
  ...
  return (
    <div className="bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm">
      ...
      <div className={`p-6 ${cfg.bg} border-b ${cfg.border}`}>
        <div className="flex items-center gap-3">
          <div
            className={`w-12 h-12 rounded-2xl ${cfg.bg} border ${cfg.border} flex items-center justify-center`}
          >
          ...
          <div className="ml-auto text-right">
            ...
            <p className="text-[11px] font-black text-gray-600 font-mono">
              ...
              return (
                <div key={step.id} className="flex items-center flex-1">
                  <div className="flex flex-col items-center gap-1.5">
                    ...
                    <span
                      className={`text-[9px] font-black uppercase tracking-wider text-center leading-tight max-w-[60px] ${
                        done
                          ? "text-[#2B3220]"
                          : active
                            ? "text-gray-500"
                  ...
                  {idx < STEPS.length - 1 && (
                    <div
                      className={`flex-1 h-0.5 mx-1 mb-5 transition-all ${currentStep > step.id ? "bg-[#A1FF4D]" : "bg-gray-100"}`}
                    />
                  )}
                </div>
              );
        ...
        <div className="space-y-2">
          {[
            ...
            },
            {
              ...
              active: order.status === "COMPLETED_BY_SUPPLIER",
            },
          ].map((item, i) => (
            <div
              key={i}
              className={`flex items-start gap-3 p-3 rounded-xl ${item.active ? "bg-[#A1FF4D]/10" : "opacity-30"}`}
            >
              <div
                className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${item.active ? "bg-[#A1FF4D]" : "bg-gray-300"}`}
              />
              <div>
                <p
                  className={`text-[11px] font-bold ${item.active ? "text-[#2B3220]" : "text-gray-400"}`}
                >
                  {item.label}
                </p>
                {item.date && (
